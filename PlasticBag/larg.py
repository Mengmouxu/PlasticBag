...
def compute_total_energy():
    for i in range(n_elements):
        ...
        # J = 1.0    # 初始化体积变化率
        # # # von mise plasticity
        # temp_sig = ti.max(sig, 0.05) # add this to prevent NaN in extrem cases
        # epsilon = ti.Vector([ti.log(temp_sig[0, 0]), ti.log(temp_sig[1, 1])])
        # epsilon_hat = epsilon - (epsilon.sum() / 6)
        ...
        #     temp_sig = ti.Matrix([[ti.exp(epsilon[0]),0],[0,ti.exp(epsilon[1])]])
        #     new_sig = temp_sig
        #     F = U @ new_sig @ V.transpose()


        # for d in ti.static(range(dim)):    # J = det(F)
        #     J *= temp_sig[d, d]
